...
export async function POST(request: NextRequest) {
	...
	if (!emailResult.success) {
		return NextResponse.json(
			{ type: 'about:blank', title: 'E-mail mislukt', status: 500, detail: `Kon bevestigingsmail niet versturen: ${emailResult.error}` },
			{ status: 500 },
		);
	}
	...
	return NextResponse.redirect(
		new URL(`/?status=email_sent&domain=${encodeURIComponent(domain)}&email=${encodeURIComponent(email)}`, request.url),
	);
}
